...
BigDecimal_uplus(VALUE self)
{
	...
	static VALUE
BigDecimal_magnitude(VALUE self, VALUE r)
{
	ENTER(5);
	Real *c, *a;
	SIGNED_VALUE n;
	...
	static VALUE
BigDecimal_magnitude2(VALUE self, VALUE b, VALUE n)
{
	ENTER(2);
	Real *cv;
	SIGNED_VALUE mx = GetPositiveInt(n);
	if (mx == 0) return BigDecimal_magnitude(self, b);
	else {
		size_t pl = VpSetPrecLimit(0);
		VALUE   c = BigDecimal_magnitude(self, b);
		VpSetPrecLimit(pl);
...
Init_bigdecimal(void)
{
	...
	rb_define_method(rb_cBigDecimal, "magnitude", BigDecimal_magnitude, 1);
	rb_define_method(rb_cBigDecimal, "safe_magnitude", BigDecimal_magnitude2, 2);
